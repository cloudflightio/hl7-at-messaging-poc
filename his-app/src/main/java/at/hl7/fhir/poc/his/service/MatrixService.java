...
public class MatrixService {
    ...
    public MatrixService(RequestBundleParser requestBundleParser) {
        this.requestBundleParser = requestBundleParser;
    }

    @PostConstruct
    public void init() {
        // Configure WebClient with 50MB buffer for large FHIR bundles
    ...
    private void processEvent(JsonNode event) {
        try {
            ...
            if ("at.fhir.message".equals(msgType)) {
                ...
                // Check for media URL first (for large bundles)
                ...
                if (fhirBundle != null && !fhirBundle.isEmpty()) {
                    ...
                    // Parse and save CommunicationRequest bundles
                    requestBundleParser.parseAndSaveBundle(fhirBundle);
                }
            }
        } catch (Exception e) {
    ...
    public boolean sendFhirMessage(String fhirBundleJson) {
        ...
        try {
            // Upload the FHIR bundle as media (to handle large bundles with PDFs)
